...
#define LIGHT_THRESHOLD 100    // Threshold for light sensor (value from 0 to 1023)
#define DARKNESS_THRESHOLD 50 // Threshold below which the buzzer should beep continuously
...
void loop() {
    ...
    if (lightLevel < DARKNESS_THRESHOLD) {
        // Continuous beep in case of extreme darkness
        digitalWrite(BUZZER_PIN, HIGH); // Turn the buzzer on continuously
        beepDelay = 0; // Set delay to 0 to indicate continuous beep
    } else if (distance < DISTANCE_THRESHOLD || lightLevel < LIGHT_THRESHOLD) {
    ...
    // Output the distance, light level, and beep delay to the Serial Monitor
    ...
    // Plot the distance and light level for real-time graphing in Serial Plotter
    Serial.print("Distance: ");
    Serial.println(distance);  // Plot distance
    Serial.print("Light Level: ");
    Serial.println(lightLevel);  // Plot light level
